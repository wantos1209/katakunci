    ...
    <div class="sec_form">
        ...
        <form action="/user/edit/{{ $user->id }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="list_form">
                ...
                <input type="text" id="name" name="name" value="{{ $user->name }}" placeholder="Input Name">
            </div>
            ...
            <div class="list_form">
                ...
                <select id="site_id" name="site_id">
                    @foreach ($dataSite as $data)
                        <option value="{{ $data->id }}" {{ $data->id == $user->site_id ? 'selected' : '' }}>{{ $data->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="list_form">
                ...
                <select id="divisi" name="divisi">
                    <option value="9" {{ $user->divisi == 9 ? 'selected' : '' }}>Superadmin</option>
                    {{-- <option value="8" {{ $user->divisi == 8 ? 'selected' : '' }}>Admin</option> --}}
                    <option value="1" {{ $user->divisi == 1 ? 'selected' : '' }}>Captain</option>
                </select>
            </div>
        
            <div class="list_form">
                <span class="sec_label">Username</span>
                <input type="text" id="username" name="username" value="{{ $user->username }}" placeholder="Input Username">
            </div>
